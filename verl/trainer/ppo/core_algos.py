...
############################################################################
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_est(AdvantageEstimator.NSR)  # or simply: @register_adv_est("nsr")
def compute_nsr_outcome_advantage(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                score_mask = scores[i] < 1
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
                scores[i] = scores[i] * score_mask
            else:
                score_mask = scores[i] < 1
                scores[i] = scores[i] - id2mean[index[i]]
                scores[i] = scores[i] * score_mask
...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_est(AdvantageEstimator.PSR)  # or simply: @register_adv_est("psr")
def compute_psr_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
